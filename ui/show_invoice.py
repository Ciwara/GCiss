...
from PyQt4.QtGui import (
    QVBoxLayout, QDialog, QIcon, QGridLayout, QMessageBox,
    QPushButton, QMenu)

from Common.ui.util import formatted_number, is_int, uopen_file, show_date
from Common.ui.common import FWidget, FLabel, FPageTitle, Deleted_btt
...
class ShowInvoiceViewWidget(QDialog, FWidget):
    ...
    def __init__(self, table_p, invoice_num, parent=0, *args, **kwargs):
        ...
        vbox = QVBoxLayout()
        vbox.addWidget(FPageTitle("Vente"))

        self.table_show = ShowOrderTableWidget(parent=self)

        editbox = QGridLayout()
        xls_bicon = QIcon.fromTheme(
            'document-del', QIcon("{}xls.png".format(Config.img_cmedia)))
        pdf_icon = QIcon.fromTheme(
            'document-del', QIcon("{}pdf.png".format(Config.img_cmedia)))
        self.button_pdf = QPushButton(pdf_icon, "")
        # self.button_pdf.setFixedWidth(30)
        # self.button_pdf.setFixedHeight(30)
        self.button_xls = QPushButton(xls_bicon, "")
        ...
        editbox.addWidget(FLabel("{typ} N°: {num}".format(
            num=self.invoice.number, typ=self.invoice.type_)), 0, 0)
        editbox.addWidget(FLabel("%s le %s" % (
            self.invoice.location, show_date(self.invoice.date))), 1, 4)
        editbox.addWidget(FLabel("Doit: %s " % self.invoice.client), 1, 0)
    ...
    def printer_pdf(self):
        ...
        from pdf_invoice import pdf_view
        import os
    ...
    def cancellation(self):
        reply = QMessageBox.question(self, 'Confirmation',
                                     "<h2 style='color:red;'>Voulez vous vraiment annuler cette"
                                     " facture?</h2>",
...
class ShowOrderTableWidget(FTableWidget):
    ...
    def popup(self, pos):
        ...
        if action == delaction:
            ...
            if list_error == []:
                ...
                raise_error("Impossible de supprimer", """<h3>L'article {} :</h3>
    ...
    def extend_rows(self):
        ...
        row_num += 1
        self.setItem(row_num, 2, TotalsWidget("Totaux : "))
        self.setItem(
            row_num, 3, TotalsWidget(formatted_number(self.montant_ht)))
        row_num += 1
        self.setItem(row_num, 2, TotalsWidget("Montant payé : "))
